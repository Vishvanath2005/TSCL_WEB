...
const Report = () => {
  ...
  const currentItemsOnPage = filteredCenters
    ...
    const [entries, setEntries] = useState([]); // Stores entries
  const [page, setPage] = useState(1); // Current page
  const [entriesPerPage, setEntriesPerPage] = useState(10); // Number of entries per page
  const [totalEntries, setTotalEntries] = useState(0)

  useEffect(() => {
    fetchEntries(page, entriesPerPage);
  }, [page, entriesPerPage]);

  const fetchEntries = async (page, limit) => {
const response =axios.fetch()
    const simulatedData = {
      entries: Array.from(
        { length: limit },
        (_, index) => `Entry ${(page - 1) * limit + index + 1}`
      ),
      total: 50, // Total entries in the dataset
    };
    setEntries(simulatedData.entries);
    setTotalEntries(simulatedData.total);
  };
...
>
  ...
  <option value="20">20</option>
        ...
        <div className="bg-white h-4/5 mx-3 rounded-lg p-3">
          ...
          <div className=" rounded-lg  py-3 overflow-x-auto no-scrollbar">
            <table className="w-full mt-3 ">
              ...
              <tbody>
                {entries && currentItemsOnPage
                  .slice()
                  .reverse()
                  .map((report, entry, index) => (
        ...
        <button
          onClick={() =>
            setPage((prev) =>
              prev < Math.ceil(totalEntries / entriesPerPage) ? prev + 1 : prev
            )
          }
          disabled={page >= Math.ceil(totalEntries / entriesPerPage)}
        >
          Next
        </button>
      </div>
